...
def load_pretrained_vits(
    ...
    """Load a Coqui VITS model (pretrained or from checkpoint).

    Args:
        model_name: Coqui model name for pretrained download.
        checkpoint_path: Path to a fine-tuned checkpoint (.pth).
            If "pretrained" or None, downloads the pretrained model.
        use_cuda: Whether to place model on GPU.

    Returns:
        The raw Coqui VITS nn.Module (not wrapped in EmotionVITS yet).
    """
    from TTS.api import TTS

    if checkpoint_path is None or checkpoint_path == "pretrained":
        logger.info(f"Loading pretrained VITS: {model_name}")
        tts = TTS(model_name=model_name, gpu=use_cuda)
        vits_model = tts.synthesizer.tts_model
    else:
        checkpoint_path = str(checkpoint_path)
        logger.info(f"Loading VITS from checkpoint: {checkpoint_path}")
        # Load checkpoint into a fresh pretrained model, then override weights
        tts = TTS(model_name=model_name, gpu=use_cuda)
        vits_model = tts.synthesizer.tts_model
        state = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
        # Support both raw state_dict and wrapped EmotionVITS checkpoints
        if "model_state_dict" in state:
            state_dict = state["model_state_dict"]
        else:
            state_dict = state
        # Filter to only VITS keys (strip 'vits.' prefix if present)
        vits_state = {}
        for k, v in state_dict.items():
            if k.startswith("vits."):
                ...
                vits_state[k] = v
        vits_model.load_state_dict(vits_state, strict=False)
...
def build_emotion_vits(
    ...
    """Factory: build an EmotionVITS model for the given system.

    Args:
        system: "A", "B", or "C".
        checkpoint_path: Init checkpoint. "pretrained" or None for raw pretrained.
        use_cuda: GPU flag.
        num_emotions: Number of emotion categories.
        embedding_dim: Emotion embedding dim (must match VITS hidden_dim).
        prosody_heads: Pre-built ProsodyHeads (required for System C).
        prosody_loss_weight: Lambda for prosody aux loss.
        model_name: Coqui model identifier.

    Returns:
        Configured EmotionVITS instance.
    """
    system = system.upper()

    vits_model = load_pretrained_vits(
    ...
    model = EmotionVITS(
        ...
        use_prosody_heads=use_prosody,
        prosody_heads=prosody_heads if use_prosody else None,
        prosody_loss_weight=prosody_loss_weight,
    )

    # Load emotion/prosody weights from checkpoint if available
    if checkpoint_path and checkpoint_path != "pretrained" and Path(checkpoint_path).exists():
        state = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
        state_dict = state.get("model_state_dict", state)
        # Load only the emotion_embedding and prosody_heads keys
    ...
    logger.info(f"EmotionVITS (System {system}) built: {params}")
